...
fn calculate_corner_rotation(xcenter: f32,
                             ycenter: f32,
                             x: f32,
                             y: f32,
                             rotationrad: f32)
                             -> Point2<f32> {
    let p = Point2::new(xcenter + (x - xcenter) * rotationrad.cos() +
                        (y - ycenter) * rotationrad.sin(),
                        ycenter + (x - xcenter) * rotationrad.sin() +
                        (y - ycenter) * rotationrad.cos());
    print!("the roration {} cos: {} sin: {}",
           rotationrad,
           rotationrad.cos(),
           rotationrad.sin());
    return p;
